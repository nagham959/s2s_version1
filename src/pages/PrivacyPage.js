        ...
        <main className="flex flex-col flex-1">
          ...
          <section className="py-16 px-6 lg:px-40 bg-white dark:bg-surface-dark text-center border-b border-gray-100 dark:border-slate-700">
            <div className="mx-auto max-w-3xl flex flex-col gap-3">
              <h2 className="text-primary font-bold tracking-wide uppercase text-sm">{t('privacy.hero.eyebrow')}</h2>
              <h1 className={`text-text-main dark:text-white text-4xl md:text-5xl font-black leading-tight tracking-[-0.033em] ${textStart}`}>
                {t('privacy.hero.title')}
              </h1>
              <p className="text-text-sub dark:text-slate-400 text-sm">{t('privacy.hero.updated')}</p>
            </div>
          </section>
