        ...
        <main className="flex flex-col flex-1">
          <section className="flex flex-col justify-center py-10 lg:py-24 px-4 md:px-10 lg:px-40 bg-white dark:bg-surface-dark">
            <div className="mx-auto flex max-w-[1280px] flex-col w-full">
              <div className="flex flex-col-reverse gap-12 lg:flex-row items-center">
                ...
                <div className={`flex flex-col gap-8 lg:w-1/2 ${contentAlign}`}>
                  <div className="flex flex-col gap-6">
                    <h1 className={`text-text-main dark:text-white text-4xl font-black leading-tight tracking-[-0.033em] md:text-5xl lg:text-6xl ${textStart}`}>
                      {t('home.hero.title')}
                    </h1>
                    <h2 className={`text-text-sub dark:text-slate-400 text-lg font-normal leading-relaxed max-w-xl ml-auto lg:ml-0 ${textStart}`}>
                      {t('home.hero.subtitle')}
                    </h2>
                  </div>

                  {/* Buttons Group - Right Aligned */}
                  <div className={`flex flex-wrap gap-3 items-center w-full ${isRtl ? 'flex-row-reverse justify-end' : 'flex-row justify-start'}`}>
                    <button
                    ...
                    >
                      <span className="truncate">{t('home.hero.ctaPrimary')}</span>
                    </button>
                    <button
                      type="button"
                      onClick={(e) => openHelp(e, t('home.hero.helpTitle'), '/videos/hello.mp4')}
                ...
                <div className="w-full lg:w-1/2">
                  <div className="relative w-full aspect-square md:aspect-[4/3] rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                    <CWASAAvatarPlayer className="absolute inset-0 h-full w-full" title="Home Avatar" />
                  </div>
                </div>
          ...
          <section id="features" className="pt-20 pb-10 px-6 lg:px-40 flex justify-center bg-white dark:bg-surface-dark">
            <div className="max-w-[960px] w-full text-center">
              <h2 className="text-primary font-bold tracking-wide uppercase text-sm mb-3">{t('home.features.eyebrow')}</h2>
              <h3 className="text-text-main dark:text-white text-3xl md:text-4xl font-bold leading-tight tracking-[-0.015em]">{t('home.features.title')}</h3>
              <p className="mt-4 text-text-sub dark:text-slate-400 max-w-2xl mx-auto text-lg">{t('home.features.description')}</p>
            </div>
          </section>
          ...
          <section className="py-16 px-6 lg:px-40 bg-background-alt dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700">
            <div className="mx-auto max-w-[960px] flex flex-col gap-10">
              <div className="text-center">
                <h2 className="text-text-main dark:text-white text-3xl font-bold leading-tight tracking-[-0.015em] mb-4">{t('home.howItWorks.title')}</h2>
                <p className="text-text-sub dark:text-slate-400 text-lg">{t('home.howItWorks.subtitle')}</p>
              </div>
              <div className="grid grid-cols-[60px_1fr] gap-x-6 md:gap-x-10 px-4">
                {t('home.howItWorks.steps', []).map((step, i, arr) => (
                  <React.Fragment key={i}>
                    ...
                    <div className={`flex flex-1 flex-col py-2 ${i < arr.length - 1 ? 'pb-12' : ''}`}>
                      ...
                      <p className="text-text-sub dark:text-slate-400 text-base leading-relaxed">{step.description}</p>
                    </div>
                  </React.Fragment>
                ))}
              </div>
          ...
          <section className="py-20 px-6 lg:px-40 bg-white dark:bg-surface-dark">
            <div className="mx-auto max-w-4xl bg-primary rounded-3xl p-10 md:p-16 text-center text-white shadow-2xl relative overflow-hidden ring-1 ring-white/10">
              ...
              <h2 className="text-3xl md:text-5xl font-black mb-6 relative z-10">{t('home.cta.title')}</h2>
              <p className="text-orange-100 text-lg mb-8 max-w-2xl mx-auto relative z-10">{t('home.cta.description')}</p>
              <div className={`flex flex-col sm:flex-row gap-4 relative z-10 w-full ${isRtl ? 'items-end justify-end sm:justify-end sm:flex-row-reverse' : 'items-center justify-center'}`}>
                <button
