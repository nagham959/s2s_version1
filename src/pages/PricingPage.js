        ...
        <main className="flex flex-col flex-1">
          ...
          <section className="py-20 px-6 lg:px-40 bg-white dark:bg-surface-dark text-center">
            <div className="mx-auto max-w-3xl">
              <h2 className="text-primary font-bold tracking-wide uppercase text-sm mb-3">{t('pricing.hero.eyebrow')}</h2>
              ...
              <div className="inline-flex items-center gap-3 bg-gray-100 dark:bg-slate-800 rounded-xl p-1">
                ...
                >
                  ...
                  <span className="text-xs bg-primary text-white rounded-full px-2 py-0.5">{t('pricing.toggle.save')}</span>
                </button>
              </div>
            </div>
          </section>
                ...
                >
                  ...
                  <div>
                    {plan.price.monthly === null ? (
                      <span className={`text-4xl font-black ${plan.highlight ? 'text-white' : 'text-text-main dark:text-white'}`}>{plan.cta}</span>
                    ) : (
                      <div className="flex items-end gap-1">
                        ...
                        {plan.price.monthly > 0 && (
                          <span className={`text-sm mb-2 ${plan.highlight ? 'text-orange-100' : 'text-text-sub dark:text-slate-400'}`}>{perMonthLabel}</span>
                        )}
                      </div>
                    )}
                  </div>
          ...
          <section className="py-16 px-6 lg:px-40 bg-background-alt dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700">
            <div className="mx-auto max-w-[800px]">
              <h2 className="text-text-main dark:text-white text-3xl font-bold text-center mb-10">{t('pricing.faq.title')}</h2>
